# SCM Price History
Accessing the price history of an item on the Steam Community Market (SCM) can be helpful for seeing the general trend of sales for an item.

It can be done manually via accessing the item's market page, an example being `https://steamcommunity.com/market/listings/730/The%20Elite%20Mr.%20Muhlik%20%7C%20Elite%20Crew`, or programmatically in `JSON` form by doing a `GET` request to `https://steamcommunity.com/market/pricehistory/?appid=APPID&market_hash_name=MARKET_HASH_NAME` with a logged-in `steamcommunity.com` session.
